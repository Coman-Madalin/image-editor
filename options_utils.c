...
{
	*sum += (double) (color[i - 1][j - 1][which] * kernel[0][0]) / coef;
	*sum += (double) (color[i - 1][j][which] * kernel[0][1]) / coef;
	*sum += (double) (color[i - 1][j + 1][which] * kernel[0][2]) / coef;
	*sum += (double) (color[i][j - 1][which] * kernel[1][0]) / coef;
	*sum += (double) (color[i][j][which] * kernel[1][1]) / coef;
	*sum += (double) (color[i][j + 1][which] * kernel[1][2]) / coef;
...
void APPLY_UTIL(int coef, int kernel[3][3], PLACEHOLDER **data)
{
	...
	for (i = (*data)->y1 + 1; i < (*data)->y2 - 1; i++) {
		for (j = (*data)->x1 + 1; j < (*data)->x2 - 1; j++) {
			...
			(*data)->image->color[i][j][2] = (int) sum2;
